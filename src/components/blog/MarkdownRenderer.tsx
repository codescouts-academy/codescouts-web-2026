...
const MarkdownRenderer = ({ content }: MarkdownRendererProps) => {
  return (
    ...
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }) => (
          <h1 className="text-3xl md:text-4xl font-bold mt-10 mb-6 text-foreground">
            {children}
          </h1>
        ),
        h2: ({ children }) => (
          <h2 className="text-2xl md:text-3xl font-bold mt-8 mb-4 text-foreground border-b border-border/50 pb-2">
            {children}
          </h2>
        ),
        h3: ({ children }) => (
